...
- a private S3 bucket and runtime-role `s3:PutObject` permission restricted to
  `benchmarks/exact-job/*`
...
Each lane checkpoints its result to a run-scoped S3 object after its fresh job
and after every resumed job. Uploads happen after the workload timer stops. The
host downloads the ten final JSON objects; benchmark data is not transported in
shell output.
