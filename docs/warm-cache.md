...
idle suspension, scheduled reconciliation, and Lambda's maximum duration are
independent backstops. DynamoDB TTL is record cleanup only and is never treated
as a MicroVM termination mechanism.
...
- `release`: verify that the JIT runner is no longer processing a job, suspend
  the MicroVM, and make it available for reuse.
- `destroy`: terminate the MicroVM and remove its warm-cache state.
- `gc`: reconcile expired leases, stale table entries, and orphaned MicroVMs.
  This is introduced in Phase 2.
...
| `ttl`                        | Eventual removal of terminal table records only                      |
...
4. If the item is `READY`, conditionally change it to `LEASED`, set a new lease
   ID, and increment the generation.
...
7. Confirm the MicroVM exists, is `SUSPENDED` or recoverably `RUNNING`, matches
   the exact image version, and has enough lifetime remaining.
8. Resume it when required, create a new JIT registration, deliver it through
   the control endpoint, and wait for the exact runner identity.
9. On an unrecoverable or expired candidate, conditionally mark it `DEAD`,
   terminate it idempotently, and create a replacement.
...
3. Conditionally change `LEASED` to `SUSPENDING` only when the lease ID and
   generation match.
4. Suspend the MicroVM and poll until AWS reports `SUSPENDED`.
5. Conditionally change `SUSPENDING` to `READY`, clear lease ownership, and
   record `lastUsedAt`.
6. If the condition fails, do not suspend the MicroVM. A newer owner may have
   acquired it.

### Reconciliation and garbage collection

`gc` is safe to run from a scheduled GitHub workflow and on demand. It:

- repairs `CREATING`, `SUSPENDING`, and `DESTROYING` operations that exceeded
  their deadlines;
- marks missing or platform-terminated MicroVMs `DEAD`;
- terminates expired MicroVMs that still exist;
- removes or ignores entries for inactive image versions;
- never mutates an unexpired lease;
- uses the same conditional generation checks as acquire and release.

Lambda's maximum duration remains the final compute cleanup boundary. DynamoDB
TTL may eventually remove `DEAD` records, but does not perform reconciliation.
...
- Cancellation and expired leases recover without manual table edits.
...
- conditional acquire, idempotent retry, release, destroy, and reconciliation;
- fencing behavior with stale lease IDs and generations;
- timestamps at maximum duration, lease expiry, and clock boundaries;
...
- lease expiration followed by reconciliation and reacquisition;
- scheduled `gc` while a valid lease is active;
...
| Workflow is cancelled       | Lease expires; platform duration and `gc` are backstops                         |
